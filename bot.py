...
async def help(ctx):
    ...
    help.add_footer(name=f"Requested by {ctx.author}",icon_url=ctx.author.display_avatar.url)    
    await ctx.send(embed=help)
@client.command()
async def info(ctx):
    embed = nextcord.Embed(title="PoopBot", description="Information", color=nextcord.Colour.green())
    embed.add_field("name="Server count", value=f"PoopBot is in {len(client.guilds)} Servers!", inline=False)
    embed.add_field(name="Prefix", value="Default prefix is p! and mentioning the bot", inline=False)
    embed.add_field(name="Help", value="To get help with PoopBot use p!help", inline=False)
    embed.add_footer(name=f"Requested by {ctx.author}",icon_url=ctx.author.display_avatar.url)                    
    await ctx.send(embed=embed)
client.run("TOKEN")
